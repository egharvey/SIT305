            ...
            for(int i = 0; i < accountList.size(); i++){
                if (accountList.get(i).getUsername().equals(username)){
                    if(accountList.get(i).getPassword().equals(password)){
                        addHistory(new History("Which of the following is not valid?", "if(x = y)", "if(x == y)", "if(x === y)", "When do you need to specify a type before assigning a value?", "Never", "Somtimes", "Always", "How do you begin a single-line comment?", "#", "//", "/*", "a", "c", "b", "a", "b", "c", 1));
